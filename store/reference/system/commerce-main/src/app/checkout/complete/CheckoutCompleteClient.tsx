    ...
    const verifyMutation = useMutation({
        mutationFn: async () => {
            ...
            if (!response.ok) {
                ...
                console.error('Verify response error:', response.status, error);
                throw new Error(error.error || error.message || 'فشل التحقق من الدفع');
            }

            const data = await response.json();
            console.log('Verify response:', data);

            // Also check logical success (server may return 200 with success: false)
            if (data && data.success === false) {
                throw new Error(data.message || 'فشل التحقق من الدفع');
            }
        ...
        onSuccess: (data) => {
            ...
            cartClient.clearCart().then(() => {
                cartClient.fetchCart().catch((e) => console.warn('Silencing cart refresh error on checkout completion', e)); // Refresh the cart state to notify all listeners
