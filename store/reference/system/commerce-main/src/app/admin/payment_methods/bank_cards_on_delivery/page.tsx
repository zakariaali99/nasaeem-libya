...
const BankCardsOnDeliveryConfigSchema = z.object({
...
export default function BankCardsOnDeliveryConfigPage() {
    ...
    const { data: paymentMethod, isLoading, error } = useQuery({
        queryKey: ['payment-method', PaymentMethodCode.BANK_CARDS_ON_DELIVERY],
        queryFn: async () => {
            const response = await fetch(`/api/payment_methods/${PaymentMethodCode.BANK_CARDS_ON_DELIVERY}`);
            if (!response.ok) {
    ...
    const formik = useFormik({
        ...
        validate: (values) => {
            const result = BankCardsOnDeliveryConfigSchema.safeParse(values);
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                queryClient.invalidateQueries({ queryKey: ['payment-method', PaymentMethodCode.BANK_CARDS_ON_DELIVERY] });
        ...
        return (
            <div className="flex items-center justify-center h-full p-8">
                <div className="text-center">
                    <h2 className="text-xl font-semibold mb-2">جاري تحميل إعدادات البطاقة المصرفية عند التوصيل...</h2>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="container p-6 mx-auto" dir="rtl">
                <Alert variant="destructive" className="mb-4">
                    <AlertDescription>
                        حدث خطأ أثناء تحميل إعدادات الدفع. يرجى المحاولة مرة أخرى.
                    </AlertDescription>
                </Alert>
            ...
            {showCreateAlert && (
                <Alert className="mb-4 border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950">
                    <AlertDescription className="text-blue-800 dark:text-blue-200">
                        🚀 مرحباً بك! لا توجد إعدادات مُكونة حالياً لطريقة الدفع هذه. املأ النموذج أدناه لإنشاء إعدادات جديدة وتفعيل الدفع ببطاقة مصرفية عند التوصيل لمتجرك.
                    </AlertDescription>
                </Alert>
            )}
            <div className="flex items-center mb-6">
                ...
                <div className="flex-1">
                    <h1 className="text-2xl font-bold">
                        {showCreateAlert ? 'إنشاء إعدادات البطاقة المصرفية عند التوصيل' : 'تعديل إعدادات البطاقة المصرفية عند التوصيل'}
                    </h1>
                    {showCreateAlert && (
                        <p className="text-sm text-muted-foreground mt-1">
                            قم بتكوين إعدادات الدفع لتمكين عملائك من الدفع عبر جهاز POS عند الاستلام
                        </p>
                    )}
                </div>
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                    <div className="bg-white dark:bg-background rounded-lg shadow-lg p-6 w-full max-w-sm text-center">
                        ...
                        <p className="mb-4">هل أنت متأكد أنك تريد حذف إعدادات الدفع؟ لا يمكن التراجع عن هذا الإجراء.</p>
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-muted-foreground">
                                أدخل تعليمات مفصلة عن الدفع، مثل كيفية استخدام بطاقة مصرفية مع مندوب التوصيل.
                            </p>
                        </div>
